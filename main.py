...
APP_TITLE = "qBittorrent Remote (Accessible)"
FILTER_CHOICES = [
    "all",
    "downloading",
    "completed",
...
class OptionsDialog(wx.Dialog):
    ...
    def __init__(self, parent: wx.Window, settings: AppSettings):
        ...
        notebook.AddPage(connection_panel, "Connection")
...
class MainFrame(wx.Frame):
    ...
    def _build_menus(self):
        ...
        delete_item = action_menu.Append(wx.ID_ANY, "Delete")
    ...
    def refresh_torrents(self, announce: bool = True):
        ...
        if not self.client.is_authenticated:
            ...
            return
        filter_value = self._filter_choice.GetStringSelection()

        def worker():
            try:
                torrents = self.client.get_torrents(filter_value)
            except Exception as exc:  # noqa: BLE001
                wx.CallAfter(self._show_error, f"Failed to refresh: {exc}")
            else:
                wx.CallAfter(self._list.update_from, torrents)
                if announce:
                    wx.CallAfter(self._set_status, f"Loaded {len(torrents)} torrents")
    ...
    def _show_about(self):
        wx.MessageBox(
            "Accessible qBittorrent remote client built with wxPython.\n"
...
def _parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Accessible qBittorrent remote client")
    parser.add_argument(
        "items",
        nargs="*",
